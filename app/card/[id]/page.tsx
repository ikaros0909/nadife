...
export async function generateMetadata(
  ...
  if (!p) return { title: "NADIPE — 나의 디지털 페르소나" };
  const w = getWorldType(p.worldType);
  const kindLabel = p.kind === "SUB" ? "숨은 부캐" : "디지털 관상";
  const title = `${w.title} · NADIPE`;
  const description = `“${p.oneLiner}” — AI가 읽어주는 나의 ${kindLabel}. NADIPE에서 이메일 하나로 60초 안에 나의 디지털 페르소나를 발견하세요.`;
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      siteName: "NADIPE",
      images: [{ url: og, width: 1200, height: 1200, alt: `${w.title} — NADIPE 디지털 관상` }],
      type: "article",
      locale: "ko_KR"
    },
    twitter: {
    ...
    <main className="relative z-10 mx-auto min-h-screen w-full max-w-xl px-6 pb-20">
      {isOwner ? (
        ...
        <header className="flex items-center justify-between py-6">
          <Link
            href="/"
            className="serif text-[10px] tracking-[0.5em] text-nadip-gold transition hover:text-nadip-glow"
          >
            NADIPE
          </Link>
          <Link
            href="/"
            className="rounded-full border border-nadip-gold/40 bg-nadip-gold/10 px-4 py-1.5 text-[11px] tracking-[0.3em] text-nadip-glow hover:bg-nadip-gold/20"
          >
            나디페 홈 →
          </Link>
        </header>
      ...
      {!isOwner && (
        <section className="mt-10 rounded-3xl border border-nadip-gold/25 bg-black/30 p-6 text-center">
          <p className="serif text-xs tracking-[0.45em] text-nadip-gold">NADIPE</p>
          <h1 className="serif mt-4 text-2xl leading-snug text-nadip-glow sm:text-3xl">
            누군가가 자신의
            <br />
            디지털 관상을 공유했어요.
          </h1>
          <p className="mt-3 text-xs leading-relaxed text-ink-100/65">
            NADIPE는 디지털 흔적을 읽는 AI 관상가입니다.
            <br />
      ...
      <div className="mt-12 rounded-3xl border border-nadip-gold/40 bg-gradient-to-br from-nadip-gold/10 to-nadip-rose/10 p-7 text-center">
        <h3 className="serif text-xl leading-snug text-nadip-glow">
