  ...
  return (
    <main className="relative z-10 mx-auto min-h-screen w-full max-w-2xl px-6 pb-20">
      ...
      <section className="pt-2">
        ...
        {feed?.iWasStarred && (
          <p className="mt-3 text-sm text-nadi-gold">✦ 이번 주, 누군가가 당신의 답신에 별표했어요.</p>
        )}
      </section>
      ...
      <section className="mt-12">
        <p className="serif text-xs tracking-[0.4em] text-nadi-gold">공중에 떠도는 한 줄</p>
...
function MyDropPanel({
  ...
  return (
    <div className="rounded-3xl border border-nadi-gold/40 bg-gradient-to-br from-nadi-gold/5 to-nadi-deep/40 p-6">
      <p className="serif text-xs tracking-[0.4em] text-nadi-gold">나의 한 줄</p>
      <p className="serif mt-3 text-base leading-relaxed text-nadi-glow">“{myDrop.text}”</p>
          ...
          {myDrop.replies.length === 0 ? (
            ...
            myDrop.replies.map((r) => (
              <div
                ...
                className={
                  r.isStarred
                    ? "rounded-xl border border-nadi-gold/60 bg-nadi-gold/15 px-4 py-3"
              ...
              >
                <div className="flex items-center justify-between text-[10px] tracking-widest text-ink-100/45">
                  <span>{r.alias}</span>
                  {r.isStarred ? (
                    <span className="text-nadi-gold">✦ 별표한 답신</span>
                  ) : !myDrop.starredReplyId ? (
                    <button
                      onClick={() => onStar(r.id)}
                      className="text-nadi-gold hover:text-nadi-glow"
                    >
                      별표
                    </button>
                  ) : null}
