    ...
    <main className="relative z-10 mx-auto min-h-screen w-full max-w-2xl px-6 pb-20">
      ...
      <section className="mt-10">
        <Link
          ...
          className="block rounded-2xl border border-nadip-gold/30 bg-gradient-to-r from-nadip-gold/10 to-nadip-rose/10 px-5 py-4 text-center text-sm tracking-[0.3em] text-nadip-glow hover:opacity-90"
        >
          ✉ 새 편지 쓰기
        </Link>
      </section>
      ...
      <section className="mt-10">
        <p className="serif text-xs tracking-[0.4em] text-nadip-gold">진행 중</p>
      ...
      {closed.length > 0 && (
        <section className="mt-12">
          <p className="serif text-xs tracking-[0.4em] text-nadip-gold/70">닫힌 편지함</p>
          <ul className="mt-4 space-y-3">
            {closed.map((t) => (
              <li key={t.id}>
    ...
    <Link
      ...
      className={
        dimmed
          ...
          : "block rounded-2xl border border-nadip-gold/25 bg-black/30 px-5 py-4 hover:-translate-y-0.5"
      }
    >
      <div className="flex items-center justify-between text-[10px] tracking-widest text-ink-100/45">
        ...
        <span className="flex items-center gap-2">
          {t.last?.unread && (
            <span className="rounded-full bg-nadip-rose px-2 py-0.5 text-[9px] text-white">새</span>
          )}
          {t.waitingForMe && t.status === "ACTIVE" && (
            <span className="text-nadip-gold">내 답장 차례</span>
          )}
          {t.status !== "ACTIVE" && <span>{t.status === "ARCHIVED" ? "마무리" : "그만둠"}</span>}
        </span>
      </div>
      <p className="serif mt-2 text-sm leading-relaxed text-nadip-glow">
