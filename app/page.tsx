  ...
  return (
    <main className="relative z-10 min-h-screen w-full">
      ...
      <section className="mx-auto flex min-h-screen max-w-4xl flex-col items-center justify-center px-6 py-24 text-center">
        <p className="serif text-sm tracking-[0.55em] text-nadip-gold">
          N · A · D · I · F · E
        </p>
        <h1 className="serif mt-10 text-5xl leading-[1.1] text-nadip-glow sm:text-7xl">
          당신은
          <br />
          <span className="bg-gradient-to-r from-nadip-gold via-nadip-rose to-nadip-gold bg-clip-text text-transparent">
            한 명이 아닙니다.
          </span>
        </h1>
        <p className="mt-10 max-w-xl text-base leading-[1.9] text-ink-100/80 sm:text-lg">
          AI가 당신의 디지털 흔적을 읽고,
          <br />
          <span className="text-nadip-glow">메인캐</span>와{" "}
          <span className="text-nadip-rose">숨어 있던 부캐</span>를 꺼냅니다.
          <br />
        ...
        <Link
          ...
          className="group mt-14 inline-flex items-center gap-3 rounded-full border border-nadip-gold/40 bg-nadip-gold/10 px-10 py-4 text-nadip-glow shadow-[0_0_40px_-10px_rgba(212,175,111,0.6)] backdrop-blur-md transition hover:bg-nadip-gold/20"
        >
          <span className="serif text-lg tracking-[0.3em]">
            {uid ? "내 화면으로" : "관상 보러 가기"}
          </span>
          <span className="text-nadip-gold transition group-hover:translate-x-1">→</span>
        </Link>

        <Link
          href={uid ? "/onboard" : "/me"}
          className="mt-4 text-xs tracking-[0.3em] text-ink-100/50 underline-offset-4 hover:text-nadip-glow hover:underline"
        >
      ...
      <section className="mx-auto max-w-3xl px-6 py-32">
        <p className="serif text-center text-xs tracking-[0.5em] text-nadip-gold/80">
          디지털 관상이란
        </p>
        <h2 className="serif mt-6 text-center text-3xl leading-snug text-nadip-glow sm:text-4xl">
          과거에는 얼굴을 보았다.
          <br />
          이제는 디지털 흔적을 본다.
        </h2>
        <div className="mt-14 space-y-6 text-center text-base leading-[1.95] text-ink-100/70">
          ...
          <p className="text-nadip-rose/90">
        ...
        <div className="mt-14 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {WORLD_TYPES.map((w) => (
            ...
            >
              ...
              <div className="serif mt-3 text-lg text-nadip-glow">{w.title}</div>
      ...
      <section className="mx-auto max-w-2xl px-6 py-32 text-center">
        <h2 className="serif text-4xl leading-snug text-nadip-glow">
          당신 안의
          <br />
        ...
        <Link
          href="/onboard"
          className="mt-12 inline-flex items-center gap-2 rounded-full bg-nadip-gold px-10 py-4 font-medium text-nadip-night transition hover:bg-nadip-glow"
        >
...
function Stat({ top, bottom }: { top: string; bottom: string }) {
  return (
    <div className="glass rounded-2xl px-3 py-4">
      <div className="serif text-xl text-nadip-glow sm:text-2xl">{top}</div>
